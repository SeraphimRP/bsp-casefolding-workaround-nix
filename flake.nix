{
    ...
    outputs = { self, nixpkgs, dream2nix }: let nixosModule = { config, lib, pkgs, ... }: {
        ...
        config = lib.mkIf config.services.bsp-casefolding-workaround.enable {
            systemd.user.services.bsp-casefolding-workaround = {
                ...
                Service = {
                    ExecStart = "${self.packages.${pkgs.system}.default}/bin/bsp-casefolding-workaround ${lib.escapeShellArgs config.services.bsp-casefolding-workaround.watch_dirs}";
                    Restart = "always";
                    Type = "simple";
                };
            };
        };
    }; eachSystem = nixpkgs.lib.genAttrs [ "x86_64-linux" ]; in {
